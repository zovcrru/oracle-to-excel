...
def _load_required_config(
    ...
    """Загружает обязательные параметры конфигурации."""
    missing_params = []
...
def _load_optional_config(
    ...
    """Загружает опциональные параметры конфигурации."""
...
def load_config(
    ...
    *,
    ...
    _load_env_file(env_path, logger)

    config: dict[str, str | int | bool] = {}
    _load_required_config(config, logger)
    _load_optional_config(config, logger)
